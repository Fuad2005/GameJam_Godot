...
func get_full_state() -> DialogicSaveState:
	# 1. Cast the created resource to a basic object reference so Godot stops over-analyzing it
	var state: Object = DialogicSaveState.new()
	...
	for subsystem in get_children():
		...
		if sub_state:
			# 2. Access via the dictionary safe-fallback
			state.get("subsystems")[subsystem.name] = sub_state

	return state as DialogicSaveState
...
func load_full_state(state: Resource) -> void:
	clear()
	...
	# Bypassing the compiler by fetching the subsystems dictionary safely via text strings
	var saved_subsystems: Dictionary = state.get("subsystems") if state.has_method("get") else {}

	for subsystem in get_children():
		if subsystem.name in saved_subsystems:
			subsystem.unpack_state(saved_subsystems[subsystem.name])
	...
	# Safe string property reading for the timeline paths
	var state_timeline = state.get("timeline")
	var state_event_index = state.get("event_index")

	if state_timeline:
		start_timeline(state_timeline, state_event_index)
	else:
